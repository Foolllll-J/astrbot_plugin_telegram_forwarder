...
- **源频道用户名列表**: 需要监听的 Telegram 公开频道 ID。
  - 支持 **指定开始时间**: `GoogleNews|2025-01-12` (表示只搬运 2025-01-12 之后的消息)。
  - 如果不加时间（如 `GoogleNews`），默认只搬运 **插件启动后** 的新消息。
- **proxy**: 这里填 http/socks5 代理地址，例如 `http://127.0.0.1:7897`。国内网络环境必须配置。
