...
class FileUploader:
    ...
    async def upload(self, fpath: str, hosting_url: str) -> Optional[str]:
        ...
        if file_size > 20 * 1024 * 1024:
            logger.info(f"File > 20MB ({file_size} bytes), using Chunked Upload for {os.path.basename(fpath)}...")
            ...
            for attempt in range(3):
                try:
                    with open(fpath, "rb") as f:
                        ...
                        resp = await client.post(hosting_url, files=files, params=params)
    ...
    async def _upload_chunked(self, uploader: httpx.AsyncClient, hosting_url: str, fpath: str) -> Optional[str]:
        ...
        try:
            ...
            m_resp = await uploader.post(hosting_url, params=m_params, data=m_data, files={'_force_multipart': ('', b'')}, timeout=60)
            ...
            if m_resp.status_code == 200:
                ...
                if "authCode=" not in check_url and "authCode=" in hosting_url:
                      try:
                          ...
                          check_url += f"&authCode={auth_code}" if "?" in check_url else f"?authCode={auth_code}"
                      except: pass
